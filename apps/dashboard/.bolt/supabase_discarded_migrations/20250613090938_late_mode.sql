/*
  ...
  1. New Tables
    ...
    - `user_business_unit_access` for fine-grained access control
    - `user_activity_log` for audit trail
    - `user_sessions` for session management
    - `user_invitations` for inviting new users
...
CREATE TABLE IF NOT EXISTS user_business_unit_access (
  ...
  UNIQUE(user_id, organization_id, COALESCE(business_unit_id, '00000000-0000-0000-0000-000000000000'::uuid))
);
...
EXCEPTION
  WHEN OTHERS THEN
    ...
    RAISE NOTICE 'Error logging user activity: %', SQLERRM;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
EXCEPTION
  WHEN OTHERS THEN
    -- Log error but don't fail the transaction
    RAISE NOTICE 'Error updating last login: %', SQLERRM;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
EXCEPTION
  WHEN OTHERS THEN
    -- Log error but don't fail the transaction
    RAISE NOTICE 'Error handling failed login: %', SQLERRM;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- IMPORTANT: First ensure all users exist in the users table
DO $$
DECLARE
  v_user_id uuid;
BEGIN
  -- First, make sure all users from organization_users exist in the users table
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'organization_users') THEN
    ...
    LOOP
      -- For each missing user, try to get info from auth.users
      DECLARE
        v_email text;
      ...
      EXCEPTION
        WHEN OTHERS THEN
          RAISE NOTICE 'Could not create user record for %: %', v_user_id, SQLERRM;
      END;
    END LOOP;
    
    -- Now that all users exist, migrate the organization_users data
    INSERT INTO user_business_unit_access (user_id, organization_id, role)
    SELECT user_id, organization_id, role
    FROM organization_users ou
    WHERE EXISTS (SELECT 1 FROM users u WHERE u.id = ou.user_id)
    ON CONFLICT (user_id, organization_id, COALESCE(business_unit_id, '00000000-0000-0000-0000-000000000000'::uuid)) 
    DO NOTHING;
  END IF;
END $$;